...
require 'nbmd_ps/core/schema_hash'
require 'nbmd_ps/core/combiner'
...
module NbmdPs
  ...
  module Combine
    ...
    # @param [Array<String>] paths
    # @param [Hash<Symbol, Object>] options
    # @return (see NbmdPs::Combiner#combine)
    def self.combine(paths, options = {})
      schemata = load_schemas(paths)
      base = NbmdPs::Template.load_json('combine_head.json')
      schema = base['$schema']
      meta = {}
      meta = NbmdPs.load_schema_file(options[:meta]) if options[:meta]
      combiner = NbmdPs::Combiner.new(meta: meta, base: base, schema: schema)
