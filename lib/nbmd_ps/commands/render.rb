...
module NbmdPs
  ...
  module Render
    ...
    # @return (see NbmdPs::Template.load_template)
    def self.get_template(options)
      ...
      NbmdPs::Template.load_template('schema.erb', template_dir)
    end
  end

  # Render provided schema to Markdown
  #
  # @param [NbmdPs::Schema] schema
  # @return [String] rendered schema in Markdown
  def self.render(schema, options = {})
    renderer = NbmdPs::Renderer.new(template: Render.get_template(options))
    doc = ''
    if options[:prepend]
      doc <<
